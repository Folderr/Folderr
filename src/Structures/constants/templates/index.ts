...
const EMAILER_TEXTS = {
	verify(username: string, verifyLink: string) {
        ...
        `Your Folderr verification link for ${verifyLink[1].split('/verify')[0]} is ${verifyLink[1]}\n` +
        'The link will expire in 48 hours.\n' +
        `If you did not request an account click this link to deny the account request: ${verifyLink[1].replace('verify', 'deny')}.`;
	},
	forgot_password(
		username: string,
		instanceLink: string,
...
const MONGOOSE = {
	expected_queries(expected: number, actual?: number) {
		return `MongooseDB > Input > findFullUser - Expected ${expected} queries in array but got ${actual ?? 'none'}`;
	}
};
