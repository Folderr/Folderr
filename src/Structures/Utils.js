...
class Utils {
    ...
    async verify(userID, asAdmin) {
        ...
        const user = new User({ uID: userID, admin: asAdmin, email: oUser.email, username: oUser.username, password: oUser.password }); // Token will be generated by user
        await user.save();
        return user;
    }
    ...
    async genValidationToken() {
        const random = crypto.randomBytes(48).toString('base64')
            .replace(/[+\\]/, '-')
            .replace(/[=/.]/, '_');
        const randomID = crypto.randomBytes(48).toString('base64')
            .replace(/[+\\]/, '-')
            .replace(/[=/.]/, '_');
