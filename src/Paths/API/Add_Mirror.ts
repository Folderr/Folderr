...
class MirrorAdd extends Path {
    ...
    async execute(req: Request, res: Response): Promise<Response> {
        ...
        if (!req.body || !req.body.url || !/http(s)?:\/\//.test(req.body.url) ) {
            return res.status(this.codes.badReq).json( { code: this.Utils.FoldCodes.mirrorInvalidUrl, message: 'Invalid Mirror URL' } );
        }
        let r,
            id,
            u;
        try {
            ...
            if (e.message && (e.message.match('Not Found') || e.message.match('[FAIL]') ) ) {
                return res.status(this.codes.notAccepted).json( { code: this.Utils.FoldCodes.mirrorReject, message: 'Mirror failed Validation' } );
            }
            return res.status(this.codes.internalErr).json( { code: this.Utils.FoldCodes.unkownError, message: 'Something unknown happened.' } );
        }
        const out = r.text;
        if (!out) {
            return res.status(this.codes.notAccepted).json( { code: this.Utils.FoldCodes.mirrorReject, message: 'Mirror failed Validation' } );
