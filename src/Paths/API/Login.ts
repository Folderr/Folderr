...
class Login extends Path {
	...
	async execute(request: Request, response: Response): Promise<Response|void> {
		if (!request.body || (request.body && (!request.body.username || !request.body.password))) {
			...
			return response.status(this.codes.badReq).json({code: this.codes.badReq, message: 'MISSING ALL DETAILS'});
		}
		...
		response.cookie('token', jwt, {expires: endTime, secure: false, httpOnly: true, sameSite: 'strict'});
		// Set cookies
