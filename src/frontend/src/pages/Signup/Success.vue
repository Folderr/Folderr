<template>
	<div class="bg-bg h-full md:h-screen flex flex-col">
		...
		<div v-else-if="err.length > 0" class="m-auto">
			<svg
		...
		<div v-else class="m-auto">
			...
			<h1
				v-if="signupType !== 'Closed'"
				class="text-brand font-bold text-4xl max-w-prose"
			>
				You have begun the sign up process for Folderr!
			</h1>
			<h1 v-else class="text-brand font-bold text-4xl">
				Sorry, signups are closed
			</h1>
			<h2
				v-if="signupType === 'Admin'"
				class="text-secondary-text text-2xl"
			>
				You now need to wait for an admin to verify your account
			</h2>
			<h2
				v-else-if="signupType === 'Email'"
				class="text-secondary-text text-2xl max-w-prose"
			>
				Please check your email to continue signing up (it may be in
				your spam/junk folder)
			</h2>
			<h2 v-else class="text-secondary-text text-2xl">
